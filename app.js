const PROJECTS = [
  ...
  { id:'create-discussion-boards', title:'Create Discussion Boards', link:'https://www.fiverr.com/techguy4/automate-your-canvas-lms-workflows-with-python-and-selenium', blurb:'Creates discussion boards with prompts and settings.', stack:['Python','Selenium'], assets:[{src:'./images/create-discussion-boards.jpg'}], details:'Batch-creates discussions with grading and availability.' },
...
function withFallback(src, text){
  if (src && src.trim() !== '') return src;
  const t = encodeURIComponent(text || 'Preview Unavailable');
  return 'https://placehold.co/1200x750?text='+t;
}
function pickCover(project){
  ...
  if (isVideo(first)) {
    ...
    return withFallback(img || first, project.title);
  }
  return withFallback(first, project.title);
}
...
function renderGrid(){
  ...
  PROJECTS.forEach(p => {
    ...
    card.className = 'card';
    card.innerHTML =
      '<div class="cover"><img alt="'+p.title+'" src="'+pickCover(p)+'"/></div>' +
      '<div class="body">' +
      ...
      '</div>';
    card.querySelector('.cover').addEventListener('click', () => openModal(p,0));
...
function renderModalMedia(){
  const p = window._openProject;
  const list = (p.assets || []).filter(Boolean);
  const safe = Math.max(0, Math.min(window._currentIndex || 0, list.length-1));
  const src = (list[safe] && list[safe].src) || '';
  ...
  if (isVideo(src)) {
    ...
    img.src = withFallback(src, p.title);
  }
  ...
  list.forEach((a,i) => {
    ...
    btn.innerHTML = isVideo(s) ? 'Video' : '<img src="'+withFallback(s,p.title)+'" alt="thumb '+(i+1)+'">';
    btn.addEventListener('click', () => { window._currentIndex = i; renderModalMedia(); });
